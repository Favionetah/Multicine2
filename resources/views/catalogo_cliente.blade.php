...
<body>
    ...
    <header>
        <a href="/catalogo" class="logo">MULTICINE</a>
        <nav>
            <ul>
                <li><a href="/cartelera">CARTELERA</a></li>
                <li><a href="/historial">MIS TICKETS</a></li>
                <li><a href="{{ route('logout') }}" style="color:var(--danger)">SALIR</a></li>
            </ul>
        </nav>
    </header>
    ...
    <script>
        ...
        function cargarPeliculas() {
            // Cambiamos a la ruta de Laravel
            fetch('/api/cartelera') 
                .then(res => res.json())
                .then(data => {
                    const grid = document.getElementById('gridPeliculas');
                    // Agrupamos por película para no repetir pósters
                    const unicas = [...new Map(data.map(item => [item['titulo'], item])).values()];
                    
                    grid.innerHTML = unicas.map(p => `
                        <div class="movie-card" onclick="verCartelera()">
                            <div class="poster-frame">
                                <img src="${p.imagen}" onerror="this.src='/img/fondo.jpg'">
                                <div class="movie-overlay"><span class="btn-buy">VER FUNCIONES</span></div>
                            </div>
