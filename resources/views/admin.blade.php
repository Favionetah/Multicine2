...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <a href="/admin/tarifas" style="display: flex; align-items: center; gap: 10px; padding: 15px 20px; background: #1a1a1a; color: white; text-decoration: none; border-radius: 10px; border: 1px solid #333; margin-bottom: 15px; font-weight: bold; transition: 0.3s;" onmouseover="this.style.background='#333'" onmouseout="this.style.background='#1a1a1a'">
            <i class="fas fa-tags" style="color: #00d2d3;"></i> Tarifas y Promociones
        </a>

        <a href="/admin/reportes" style="display: flex; align-items: center; gap: 10px; padding: 15px 20px; background: #1a1a1a; color: white; text-decoration: none; border-radius: 10px; border: 1px solid #333; margin-bottom: 15px; font-weight: bold; transition: 0.3s;" onmouseover="this.style.background='#333'" onmouseout="this.style.background='#1a1a1a'">
            <i class="fas fa-chart-pie" style="color: #00d2d3;"></i> Reportes Financieros
        </a>

        <a href="/admin/empleados" style="display: flex; align-items: center; gap: 10px; padding: 15px 20px; background: #1a1a1a; color: white; text-decoration: none; border-radius: 10px; border: 1px solid #333; margin-bottom: 15px; font-weight: bold; transition: 0.3s;" onmouseover="this.style.background='#333'" onmouseout="this.style.background='#1a1a1a'">
            <i class="fas fa-users-cog" style="color: #00d2d3;"></i> Gestión de Empleados
        </a>
        

        <a href="{{ route('logout') }}" class="side-btn btn-logout"><i class="fas fa-power-off"></i> Cerrar Sesión</a>
    </div>
            ...
            <form id="formFuncion">
                ...
                <div style="display:flex; gap:10px;">
                    ...
                    <div style="margin-top: 15px; margin-bottom: 15px;">
                        <label style="color:#888; font-size:0.8rem; font-weight:bold; text-transform:uppercase;">Fecha Fin (Opcional)</label>
                        <input type="date" name="fechaFin" style="width: 100%; padding: 12px; background: #0a0a0a; border: 1px solid #333; color: white; border-radius: 8px; font-family: inherit; margin-top: 5px;">
                        <small style="color:var(--primary); font-size:0.75rem; display:block; margin-top:5px;">* Llena esto si quieres programar el mismo horario para varios días seguidos.</small>
                    </div>
                </div>
    ...
    <script>
        ...
        // 1. Guardar Nueva Película (Actualizado a Laravel)
        document.getElementById('formPelicula').addEventListener('submit', function(e) {
            e.preventDefault();
            fetch('/api/peliculas', { method: 'POST', body: new FormData(this) })
            .then(res => res.json())
            .then(data => {
            ...
            .catch(err => console.error("Error guardando película:", err));
        });

        // 2. Cargar Catálogo (Actualizado a Laravel)
        // 2. Cargar Catálogo (A prueba de nulos)
        function cargarPeliculas() {
            ...
            .then(data => {
                ...
                grid.innerHTML = data.map(p => {
                    // Verificamos si imagenPoster existe antes de usar startsWith
                    const poster = p.imagenPoster || ''; 
        ...
        function abrirModalFuncion() {
            ...
            // 3. Traer películas (Actualizado a Laravel)
            fetch('/api/peliculas')
            .then(res => res.json())
            .then(data => {
            ...
            // 4. Traer SALAS REALES (Actualizado a Laravel)
            fetch('/api/salas')
            .then(res => res.json())
            .then(data => {
                ...
                data.forEach(s => {
                    selectSala.innerHTML += `<option value="${s.idSala}">${s.nombre} (${s.capacidadTotal} asientos)</option>`;
                });
            });
        ...
        function cerrarModalFuncion() { document.getElementById('modalFuncion').classList.remove('active'); }

        // 5. Guardar Función (Actualizado a Laravel)
        document.getElementById('formFuncion').addEventListener('submit', function(e) {
            e.preventDefault();
            fetch('/api/funciones', { method: 'POST', body: new FormData(this) })
            .then(res => res.json())
            .then(data => {
                if(data.status === 'success') {
        ...
        // 6. Guardar Sala (Actualizado a Laravel)
        document.getElementById('formSala').addEventListener('submit', function(e) {
            e.preventDefault();
            fetch('/api/salas', { method: 'POST', body: new FormData(this) })
            .then(res => res.json())
            .then(data => {
