...
<body>
    ...
    <header>
        <nav>
            <ul>
                <li><a href="catalogo_cliente.php">CATALOGO</a></li>
            </ul>
        </nav>
    ...
    <div class="modal-overlay" id="infoModal">
        <div class="modal-content" style="max-width: 650px; padding: 0; border: 1px solid var(--primary);">
            <div style="position: relative; height: 300px; border-radius: 20px 20px 0 0; overflow: hidden;">
    ...
    <script>
        ...
        function renderizarPeliculas() {
            ...
            if (funcionesDelDia.length === 0) {
                container.innerHTML = '<h3 style="text-align:center; grid-column:1/-1; color:#666; margin-top:50px;">No hay funciones programadas para esta fecha.</h3>';
                return;
            }

            const peliculasAgrupadas = {};
            funcionesDelDia.forEach(f => {
                if (!peliculasAgrupadas[f.titulo]) peliculasAgrupadas[f.titulo] = {
                    titulo: f.titulo,
                    imagen: f.imagen,
                    funciones: []
                };
            ...
            for (const titulo in peliculasAgrupadas) {
                ...
                // Se movió el botón de VER INFORMACIÓN al área de horarios
                const tituloSeguro = peli.titulo.replace(/'/g, "\\'");
                container.innerHTML += `
                    <div class="movie-card">
                        ...
                        <div class="m-info">
                            ...
                            <div class="functions-area">
                                <span class="f-label">Opciones y Horarios:</span>
                                <div class="f-buttons">
                                    <button class="btn-info-action" onclick="abrirInfoPelicula('${tituloSeguro}')">
        ...
        // --- LÓGICA PARA MOSTRAR LA INFO DE LA PELÍCULA ---
        function abrirInfoPelicula(titulo) {
            const func = funcionesGlobales.find(f => f.titulo === titulo);
            if (!func) return;

            document.getElementById('infoTitulo').textContent = func.titulo;
            document.getElementById('infoImagen').src = func.imagen;
            document.getElementById('infoGenero').textContent = func.genero || 'No especificado';
            document.getElementById('infoDuracion').textContent = func.duracion || '0';
            document.getElementById('infoClasificacion').textContent = func.clasificacion || '-';
            document.getElementById('infoIdioma').textContent = func.idioma || '-';
            document.getElementById('infoSinopsis').textContent = func.sinopsis || 'Sin sinopsis disponible para esta película.';
