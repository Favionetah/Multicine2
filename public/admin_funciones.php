...
<body>
    ...
    <script>
        ...
        function cargarSalasParaSelect() {
            fetch('api.php?action=listar_salas').then(res => res.json()).then(data => {
                const select = document.getElementById('edit_idSala');
                select.innerHTML = '';
                data.forEach(s => {
                    select.innerHTML += `<option value="${s.idSala}">${s.nombre} (${s.capacidad} asnts.)</option>`;
                });
            });
        }

        // 2. CARGAR Y DIBUJAR FUNCIONES
        function cargarFunciones() {
            fetch('api.php?action=listar_funciones')
            .then(res => res.json())
            .then(data => {
                ...
                data.forEach(f => {
                    const ruta = f.imagenPoster.startsWith('http') ? f.imagenPoster : `img/${f.imagenPoster}`;
                    container.innerHTML += `
                        <div class="function-card">
                            ...
                            <div class="info">
                                <div class="info-row"><span>SALA:</span> <strong>${f.idSala}</strong></div>
        ...
        function eliminar(id) {
            if (confirm('¿Eliminar esta función de la cartelera?')) {
                const fd = new FormData(); fd.append('action', 'eliminar_funcion'); fd.append('id', id);
                fetch('api.php', { method: 'POST', body: fd }).then(() => cargarFunciones());
            }
        }
    </script>
</body>
